...
export default function DungeonPage({ params }: { params: { id: string } }) {
  ...
  // 【新增】：时空裂缝的演出状态机 (0:字 -> 1:震动 -> 2:变大转圈 -> 3:定型打字)
  ...
  return (
    <div className="min-h-screen w-full bg-[#f1f5f9] flex flex-col items-center justify-center p-4">
      ...
      {step !== 'settlement' ? (
        // 【注意此处】：当 riftPhase === 1 时，给整个大卡片施加激烈的震动动画
              ...
              {showOptions && (
                <div className="mt-8 space-y-6 w-full animate-in slide-in-from-bottom flex flex-col items-center">
                  ...
                  <div className="w-full bg-slate-50 border-4 border-slate-900 p-6 rounded-2xl flex flex-col items-center">
                    ...
                    {showSettlement && (
                      <div id="settlement-card" className="w-full retro-paper p-8 md:p-12 rounded-xl text-left animate-in zoom-in mt-6 relative">
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="space-y-4 mb-8">
                              <p className="text-lg retro-text font-bold text-[#8b5a2b]">称号：</p>
                              <p className="text-3xl font-black text-[#8b2b2b] tracking-wider mb-6">{res.title}</p>
                              <p className="text-lg retro-text font-bold text-[#8b5a2b]">你曾经：</p>
                        ...
                        <button
                          ...
                          className="w-full mt-10 bg-[#3e3c38] text-[#fdf6e3] p-4 rounded-xl font-bold text-xl active:translate-y-1 shadow-lg flex items-center justify-center gap-2"
                        >
