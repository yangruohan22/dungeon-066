...
export default function DungeonPage({ params }: { params: { id: string } }) {
  ...
  // 【新增】：自定义 UI 提示弹窗 (Toast)
  const [toastMsg, setToastMsg] = useState('');
  const showToast = (msg: string) => {
    setToastMsg(msg);
    setTimeout(() => setToastMsg(''), 1500); // 1.5秒后自动淡出
  };
  ...
  // 动态按钮行为处理 (使用 showToast 替换 alert)
  const handleActionClick = () => {
    if (actionCount >= currentRoute.limit) {
      if (currentRoute.limitAlert) showToast(currentRoute.limitAlert);
      return;
    }
  ...
  const handleTalentPerform = () => {
    ...
    if (talents.length < 3) {
      ...
      showToast(currentRoute.talentType === 'cat' ? '小猫看厌了，转身离开' : '人群看厌了，转身离开');
      setStep('transition');
    }
  };
  ...
  const handleDownloadImage = async () => {
    ...
    if (element) {
      ...
      const canvas = await html2canvas(element, { scale: 2, backgroundColor: '#fcf5e5' });
  ...
  return (
    <div className="min-h-screen w-full bg-[#f1f5f9] flex flex-col items-center justify-center p-4">
      ...
      {/* 全局自定义 Toast 提示框 */}
      <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 pointer-events-none ${toastMsg ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'}`}>
        <div className="bg-slate-900/90 text-white px-8 py-4 rounded-2xl shadow-2xl font-bold text-lg backdrop-blur-sm">
          {toastMsg}
        </div>
      </div>
        ...
        <div className="bg-white max-w-2xl w-full border-4 border-slate-900 rounded-3xl solid-shadow p-8 md:p-12 transition-all duration-500 overflow-hidden">
          ...
          {step === 'site' && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <button onClick={() => { if(thankYouMsg) setStep('cat'); else showToast('心里话不能为空！'); }} className="w-full bg-slate-900 text-white p-4 rounded-xl font-bold text-xl active:translate-y-1">提交</button>
            </div>
          )}
          ...
          {step === 'cat' && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <button onClick={() => { setStep('cat_show_DE'); }} className="block w-full text-left bg-slate-50 border-2 border-slate-900 p-4 rounded-xl font-bold text-lg active:translate-y-1">C. 这都什么破选项，我选不出来</button>
            </div>
          )}

          {step === 'cat_show_DE' && (
            <div className="space-y-4 animate-in fade-in">
              <button onClick={() => { showToast("不是给你更多选项了吗？怎么还来压榨我。"); }} className="block w-full text-left bg-slate-50 border-2 border-slate-900 p-4 rounded-xl font-bold text-lg">C. 这都什么破选项，我选不出来</button>
          ...
          {step === 'q2_D' && (
            <div className="space-y-4 animate-in slide-in-from-right">
              ...
              <button onClick={() => showToast(`请你有自知之明一点，你还想像上次【${data.lore}】一样尴尬吗！`)} className="block w-full text-left bg-slate-50 border-2 p-4 rounded-xl font-bold border-slate-900">你却将身一扭，反从小猫的胯下逃走了</button>
            </div>
          )}
            ...
            <div className="space-y-8 flex flex-col items-center animate-in zoom-in">
              ...
              <div className="relative w-[360px] h-[360px] flex items-center justify-center">
                {Array.from({length: 16}).map((_, i) => (
                  <span key={i} className="circle-char text-3xl font-black text-blue-600" style={{ transform: `rotate(${i * (360/16)}deg) translateY(-170px)` }}>马</span>
                ))}
                <div className="text-center font-bold text-lg z-10 px-8">
          ...
          {step === 'boss' && (
            <div className="space-y-6 animate-in slide-in-from-bottom">
              <div className="text-xl font-bold leading-relaxed min-h-[120px]">
                <Typewriter text={`在通往出口的唯一道路上，站着【${data.fear_boss}】，ta威胁你，如果你回答不出下面的问题，就要【${data.fear_punish}】。`} />
              </div>
              ...
              <button onClick={() => {
                ...
                // 【修改】：使用自定义 Toast 提示错误并加上老板逼近的文案
                else {
                  setBossFails(prev => prev + 1);
                  showToast(`答案错误！【${data.fear_boss}】又近了一步！`);
                }
