      ...
      <div className={`bg-white max-w-2xl w-full border-4 border-slate-900 rounded-3xl solid-shadow p-6 md:p-12 transition-all duration-500 overflow-hidden ${step === 'time_rift' && riftPhase === 1 ? 'animate-fierce-shake' : ''}`}>
        ...
        {step === 'intro' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-xl md:text-2xl font-bold min-h-[60px]">
              <Typewriter text={`亲爱的【${data.target_name}】，恭喜你在【地球Online】解锁了新副本！`} onComplete={() => setShowOptions(true)} />
            </div>
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-bottom">
                <button onClick={() => goStep('mail')} className="block w-full bg-blue-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">立即探索</button>
                <button onClick={() => goStep('abandon')} className="block w-full bg-slate-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">放弃探索</button>
                <button onClick={() => showToast("那不好意思了，这个选项也无法选中~")} className="block w-full bg-slate-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">只要我不表态，你就无法选中我</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'mail' && (
          <div className="text-center space-y-6 animate-in zoom-in">
            ...
            <div className="text-xl md:text-2xl font-bold min-h-[40px]">
              <Typewriter text={`你收到了一封邮件`} onComplete={() => setShowOptions(true)} />
            </div>
            {showOptions && (
              <button onClick={() => goStep('intro')} className="w-full bg-blue-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl animate-in slide-in-from-bottom">打开</button>
            )}
          </div>
        )}
        ...
        {step === 'abandon' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            {showOptions && (
              <button onClick={() => goStep('site')} className="w-full bg-blue-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1 animate-in slide-in-from-bottom">好吧</button>
            )}
          </div>
        )}
        ...
        {step === 'site' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-lg md:text-xl font-bold leading-relaxed min-h-[80px]">
        ...
        {step === 'cat' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-lg md:text-xl font-bold min-h-[80px]">
              <Typewriter text={`一只会读心术的小猫听到了你的心里话，优雅地走过来：喵喵喵！（你心里话的声音好大！）\n此时你：`} onComplete={() => setShowOptions(true)} />
            </div>
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-bottom">
                <button onClick={() => goStep('q2_A')} className="block w-full text-left bg-slate-50 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">A. 尝试友好交流：喵喵喵！（%#@-#&￥@！）</button>
                <button onClick={() => goStep('q2_B')} className="block w-full text-left bg-slate-50 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">B. 什么？小猫！一把抱走</button>
                <button onClick={() => {
                  if (!hasClickedC) {
                    setHasClickedC(true);
                ...
                }} className="block w-full text-left bg-slate-50 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">C. 这都什么破选项，我选不出来</button>

                {showAllCatOptions && (
                  <div className="space-y-4 animate-in fade-in duration-500 pt-2">
                    <button onClick={() => goStep('q2_D')} className="block w-full text-left bg-yellow-50 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">D. 被吓到</button>
                    <button onClick={() => goStep('q2_E')} className="block w-full text-left bg-yellow-50 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">E. 不知从哪里掏出一把猫粮</button>
                  </div>
                )}
              </div>
            )}
        ...
        {step === 'q2_A' && (
          <div className="space-y-4 animate-in fade-in">
            <div className="text-lg md:text-xl font-bold min-h-[60px]">
              <Typewriter text={`这是一只贪婪的小猫，它发现你很友好，于是蹬鼻子上脸，坐在了你的头上：`} onComplete={() => setShowOptions(true)} />
            </div>
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-right">
                <button onClick={() => { setRoute(1); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">挥舞着【{data.fear_boss}】试图驱赶小猫</button>
                <button onClick={() => { setRoute(2); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">把【{data.item}】放在前面，引诱小猫下来</button>
                <button onClick={() => { setRoute(3); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">我的小猫我来宠，原地表演【{data.lore}】</button>
                <button onClick={() => showToast("小猫虽好，不要贪杯哦~")} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">深吸一口，被小猫味搞得晕头转向</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'q2_B' && (
          <div className="space-y-4 animate-in fade-in">
            ...
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-right">
                <button onClick={() => { setRoute(4); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">从兜里掏出【{data.fear_boss}】试图驱赶人群</button>
                <button onClick={() => { setRoute(5); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">悄悄问旁边的人要不要【{data.item}】，企图贿赂ta</button>
                <button onClick={() => { setRoute(6); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">已经这么尴尬了，还能怎么样？开始表演【{data.lore}】</button>
                <button onClick={() => showToast("嗯对。")} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">只要我不尴尬，尴尬的就是别人</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'q2_D' && (
          <div className="space-y-4 animate-in fade-in">
            <div className="text-lg md:text-xl font-bold min-h-[40px]">
              <Typewriter text={`小猫嘿嘿一笑：`} onComplete={() => setShowOptions(true)} />
            </div>
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-right">
                <button onClick={() => { setRoute(7); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">愣在原地，看着小猫一步一步朝自己走来</button>
                <button onClick={() => { setRoute(8); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">拿出【{data.item}】，放在自己和小猫中间</button>
                <button onClick={() => { setRoute(9); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">把小猫介绍给【{data.fear_boss}】</button>
                <button onClick={() => showToast(`请你有自知之明一点，你还想像上次【${data.lore}】一样尴尬吗！`, 3000)} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">你却将身一扭，反从小猫的胯下逃走了</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'q2_E' && (
          <div className="space-y-4 animate-in fade-in">
            ...
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-right">
                <button onClick={() => { setRoute(10); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">我是让你吃猫粮啊，你在吃什么？！</button>
                <button onClick={() => { setRoute(11); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">从兜里掏出【{data.fear_boss}】，想让小猫把ta也吃掉</button>
                <button onClick={() => { setRoute(12); goStep('action'); }} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">拿出【{data.item}】，也装模作样吃了起来</button>
                <button onClick={() => showToast("真的吗。。。")} className="block w-full text-left bg-slate-50 border-2 p-3 md:p-4 rounded-xl font-bold border-slate-900 text-base md:text-lg">真好啊，让你忘了那次【{data.lore}】有多尴尬</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'action' && currentRoute && (
          <div className="space-y-8 animate-in fade-in relative">
            <div className="text-xl md:text-2xl font-bold leading-relaxed min-h-[120px]">
              ...
              <div className="animate-in slide-in-from-bottom">
                <div className="flex justify-center relative h-20 mb-8">
                  <button
                    ...
                    className="bg-red-100 border-4 border-slate-900 px-6 py-3 md:px-8 md:py-4 rounded-2xl font-black text-lg md:text-xl text-red-600 active:scale-95 z-10"
                  >
          ...
          <div className="space-y-6 animate-in fade-in">
            ...
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-bottom">
                ...
                <input maxLength={15} value={currentTalent} onChange={e=>setCurrentTalent(e.target.value)} className="w-full border-b-4 border-slate-900 p-3 text-lg md:text-xl outline-none" placeholder="输入才艺(15字以内)"/>
                <button onClick={handleTalentPerform} className="w-full bg-slate-900 text-white p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">表演</button>

                {talents.length >= 3 && (
                  <button onClick={() => goStep('transition')} className="w-full mt-4 bg-yellow-100 border-2 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">
        ...
        {step === 'transition' && (
          <div className="space-y-8 text-center animate-in fade-in">
            <div className="text-xl md:text-2xl font-bold min-h-[80px]">
            ...
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-bottom">
                <button
                  ...
                  className={`w-full bg-slate-100 border-4 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl transition-transform duration-500 ${isRotated ? 'rotate-180' : ''}`}
                >
                  我倒要看看
                </button>
                <button onClick={() => goStep('time_rift')} className="w-full bg-blue-100 border-4 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">放马过来吧</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === 'time_rift' && (
          <div className="space-y-8 flex flex-col items-center justify-center min-h-[350px]">
            ...
            {riftPhase < 2 && (
              <div className="text-3xl md:text-4xl font-black text-red-600 text-center animate-in zoom-in w-full">
            ...
            {riftPhase >= 2 && (
              <div className="relative w-[280px] h-[280px] md:w-[360px] md:h-[360px] flex items-center justify-center animate-in fade-in duration-500">
                ...
                <div className="text-center font-bold text-lg md:text-xl z-10 px-8 md:px-10 leading-loose text-slate-800">
                  {riftPhase === 3 && (
            ...
            {showOptions && riftPhase === 3 && (
              <button onClick={() => goStep('boss')} className="w-full bg-slate-900 text-white p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1 animate-in zoom-in mt-4">没招了</button>
            )}
          </div>
        )}
        ...
        {step === 'boss' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-lg md:text-xl font-bold leading-relaxed min-h-[120px]">
            ...
            {showOptions && (
              <div className="space-y-4 animate-in slide-in-from-bottom">
                ...
                }} className="w-full bg-slate-900 text-white p-3 md:p-4 rounded-xl font-bold text-lg md:text-xl active:translate-y-1">提交答案</button>

                {bossFails >= 3 && (
                  <button onClick={() => setBossInput(data.spell_answer)} className="w-full mt-4 bg-yellow-100 border-4 border-slate-900 p-3 md:p-4 rounded-xl font-bold text-base md:text-lg active:translate-y-1">
            ...
            {showOptions && (
              <div className="mt-8 space-y-6 w-full animate-in slide-in-from-bottom flex flex-col items-center">
                ...
                <div className="w-full bg-slate-50 border-4 border-slate-900 p-4 md:p-6 rounded-2xl flex flex-col items-center">
                  <p className="font-bold text-slate-500 mb-4 text-sm md:text-base">Ta给你留了一个小纸条……</p>
                  <button onClick={() => setShowPaper(!showPaper)} className="text-5xl md:text-6xl hover:scale-110 transition-transform mb-2">✉️</button>

                  {showPaper && (
                    <div id="note-card" className="sticky-note p-8 md:p-10 mt-6 min-h-[140px] w-full sm:w-[90%] max-w-sm transform -rotate-2 animate-in zoom-in text-left relative flex flex-col justify-center text-slate-800">
                      <p className="text-xl md:text-2xl font-bold leading-relaxed">{data.final_message}</p>
                      <div className="w-full text-right mt-6 font-black text-slate-700 text-base md:text-lg">
                        —— {data.creator_nick}
                      </div>
                      <button id="dl-note-btn" onClick={handleDownloadNote} className="absolute bottom-3 right-4 text-slate-400 hover:text-slate-600 font-bold text-xs md:text-sm">
                        💾 保存
                      </button>
                    </div>
                  )}
                ...
                <div className="w-full flex flex-col items-center mt-8">
                  ...
                  {showSettlement && (
                    <div id="settlement-card" className="w-full retro-paper border-4 border-slate-900 p-6 md:p-12 rounded-xl text-left animate-in zoom-in mt-6 relative overflow-hidden shadow-xl">
                      ...
                      <p className={`${valueClass} mt-2 md:mt-3`}>
                        “{thankYouMsg}”
                      </p>

                      <p className="text-center font-black text-lg md:text-xl text-[#6b8e23] pt-4 md:pt-6 border-t-2 border-[#d4c4af]">
                        本次副本闯关到此全部结束，请收拾好您的行李物品，回归主线任务！加油！
                      </p>

                      <div className="mt-6 md:mt-10 pt-4 border-t-2 border-dashed border-[#d4c4af] flex flex-col sm:flex-row justify-between items-center opacity-70 gap-2">
                        <p className={`${labelClass} mb-0 text-xs md:text-sm`}>地球Online·专属人生副本</p>
                        <p className={`${labelClass} mb-0 text-xs md:text-sm`}>前往 bazinga66.top 为损友定制</p>
                      </div>
